...
public class NoRender extends Module_eSdgMXWuzcxgQVaJFmKZ {
   ...
   public NoRender() {
      super("NoRender", Module_JlagirAibYQgkHtbRnhw.Render);
   ...
   public void onRender3D(MatrixStack matrixStack, float partialTicks) {
      for (Entity ent : mc.field_1687.method_18112()) {
         if (ent instanceof PotionEntity && this.potions.getValue()) {
            mc.field_1687.method_2945(ent.method_5628(), RemovalReason.field_26998);
         }
   ...
   public void onParticle(ParticleEvent event) {
      if (this.elderGuardian.getValue() && event.particle instanceof ElderGuardianAppearanceParticle) {
         ...
         event.setCancelled(true);
      } else if (!this.fireworks.getValue() || !(event.particle instanceof FireworkParticle) && !(event.particle instanceof Flash)) {
         if (this.effect.getValue() && event.particle instanceof SpellParticle) {
            event.cancel();
         }
      } else {
         event.setCancelled(true);
