...
public class TwoDESP_CLphFghCvliwVuLcyYHt extends Module_eSdgMXWuzcxgQVaJFmKZ {
   private final EnumSetting page = this.add(new EnumSetting("Settings", TwoDESP.Target));
   public final ColorSetting armorDuraColor = this.add(new ColorSetting("Armor Dura Color", new Color(3145472), v -> this.page.getValue() == TwoDESP.Color));
   ...
   private final BooleanSetting drawItemC = this.add(
      new BooleanSetting("draw Item Count", true, v -> this.page.getValue() == TwoDESP.Setting && this.drawItem.getValue())
   );
   public final ColorSetting countColor = this.add(
      new ColorSetting("Item Count Color", new Color(255, 255, 0, 255), v -> this.page.getValue() == TwoDESP.Color && this.drawItemC.getValue())
   );
   public final ColorSetting textcolor = this.add(
      new ColorSetting("Item Name Color", new Color(255, 255, 255, 255), v -> this.page.getValue() == TwoDESP.Color && this.drawItem.getValue())
   );
   ...
   private final ColorSetting friendsC = this.add(new ColorSetting("FriendsBox", new Color(3211008), v -> this.page.getValue() == TwoDESP.Color));
   ...
   private final ColorSetting monstersC = this.add(new ColorSetting("MonstersBox", new Color(16711680), v -> this.page.getValue() == TwoDESP.Color));
   ...
   public void onRender2D(DrawContext context, float tickDelta) {
      ...
      for (Entity entx : mc.field_1687.method_18112()) {
         if (this.shouldRender(entx)) {
            this.drawText(entx, context);
         }
      }
   }
   ...
   public boolean shouldRender(Entity entity) {
      if (entity == null) {
         ...
         if (entity == mc.field_1724 && mc.field_1690.method_31044().method_31034()) {
            return false;
         } else {
   ...
   public Color getEntityColor(Entity entity) {
      if (entity == null) {
         ...
         return me.hextech.HexTech.FRIEND.isFriend((PlayerEntity)entity) ? this.friendsC.getValue() : this.playersC.getValue();
      } else if (entity instanceof EndCrystalEntity) {
         return this.crystalsC.getValue();
      } else {
         return switch (entity.method_5864().method_5891()) {
            case field_6294, field_6300 -> this.creaturesC.getValue();
            case field_6302 -> this.monstersC.getValue();
            case field_6303, field_24460 -> this.ambientsC.getValue();
   ...
   public void drawBox(BufferBuilder bufferBuilder, @NotNull Entity ent, Matrix4f matrix, DrawContext context) {
      ...
      if (position != null) {
         ...
         if (ent instanceof LivingEntity lent && lent.method_6032() != 0.0F && this.renderHealth.getValue()) {
            Render2DUtil.setRectPoints(
               bufferBuilder, matrix, (float)(posX - 4.0), (float)posY, (float)posX - 3.0F, (float)endPosY, Color.BLACK, Color.BLACK, Color.BLACK, Color.BLACK
            );
            ...
            Render2DUtil.setRectPoints(
               ...
               matrix,
               (float)(posX - 4.0),
               (float)(endPosY + (posY - endPosY) * (double)lent.method_6032() / (double)lent.method_6063()),
               (float)posX - 3.0F,
               (float)endPosY,
               color,
               color,
         ...
         if (ent instanceof PlayerEntity player && this.renderArmor.getValue()) {
            ...
            for (ItemStack armor : stacks) {
               ...
               if (!armor.method_7960()) {
                  float durability = (float)(armor.method_7936() - armor.method_7919());
                  ...
                  Render2DUtil.setRectPoints(
                     ...
                     matrix,
                     (float)(endPosX + 1.5),
                     (float)((double)((float)posY) + height * (double)i + 1.2 * (double)(i + 1)),
                     (float)endPosX + 3.0F,
                     (float)((int)(posY + height * (double)i + 1.2 * (double)(i + 1) + finalH)),
                     this.armorDuraColor.getValue(),
                     this.armorDuraColor.getValue(),
                     this.armorDuraColor.getValue(),
                     this.armorDuraColor.getValue()
                  );
   ...
   public void drawText(Entity ent, DrawContext context) {
      ...
      if (position != null) {
         ...
         if (ent instanceof ItemEntity entity && this.drawItem.getValue()) {
            ...
            context.method_51433(mc.field_1772, entity.method_5476().getString(), (int)tagX, (int)(posY - 10.0), this.textcolor.getValue().getRGB(), false);
            if (this.drawItemC.getValue()) {
               context.method_51433(
                  mc.field_1772,
                  "x" + count,
                  (int)(tagX + (float)mc.field_1772.method_1727(entity.method_5476().getString() + " ")),
                  (int)posY - 10,
                  this.countColor.getValue().getRGB(),
                  false
               );
            }
         }
