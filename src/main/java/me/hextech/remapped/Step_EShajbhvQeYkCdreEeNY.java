...
import me.hextech.remapped.Step;
...
extends Module_eSdgMXWuzcxgQVaJFmKZ {
    ...
    private final EnumSetting<Step> mode = this.add(new EnumSetting<Step>("Mode", Step.Vanilla));
    boolean timer;
    int packets = 0;
    ...
    public double[] getOffset(double height) {
        boolean strict;
        boolean bl = strict = this.mode.getValue() == Step.NCP;
        if (height == 0.75) {
            if (strict) {
