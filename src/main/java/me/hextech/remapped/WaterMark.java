...
public class WaterMark extends Module_eSdgMXWuzcxgQVaJFmKZ {
   ...
   private FontAdapter createFontAdapter(float size, String name) throws Exception {
      return new RendererFontAdapter(
         Font.createFont(0, (InputStream)Objects.requireNonNull(WaterMark.class.getClassLoader().getResourceAsStream("assets/nullpoint/font/" + name + ".ttf")))
            .deriveFont(0, size),
         size
      );
   }
   ...
   public void onRender2D(DrawContext drawContext, float tickDelta) {
      if (!nullCheck()) {
         ...
         if (this.fps.getValue()) {
            String fpsString = String.valueOf(me.hextech.HexTech.FPS.getFps());
         ...
         float x = (float)this.offset.getValueInt();
         float y = (float)this.offset.getValueInt();
         if (this.background.getValue()) {
            int textWidth = this.customFont.getValue() && this.lexendFont != null
               ? (int)this.lexendFont.getStringWidth(textToRender)
               : mc.field_1772.method_1727(textToRender);
            int textHeight = this.customFont.getValue() && this.lexendFont != null ? (int)this.lexendFont.getFontHeight() : 9;
            float bgWidth = (float)(textWidth + 175);
            float bgHeight = (float)(textHeight + 10);
            ...
            if (this.round.getValueFloat() > 0.0F) {
               this.renderRoundedQuad(
                  ...
                  (double)y,
                  (double)(x + bgWidth),
                  (double)(y + headerHeight + this.round.getValueFloat()),
                  (double)this.round.getValueFloat(),
                  (double)this.round.getValueFloat(),
                  0.0,
                  0.0,
                  32.0
            ...
            if (this.customFont.getValue() && this.lexendFont != null) {
               ...
               drawContext.method_25303(mc.field_1772, textToRender, (int)(x + 5.0F), (int)(y + (bgHeight - (float)textHeight) / 2.0F), -1);
            }
            ...
            drawContext.method_25303(mc.field_1772, textToRender, this.offset.getValueInt(), this.offset.getValueInt(), -1);
         }
      }
   }

   private void renderRoundedQuad(
      MatrixStack matrices, Color c, double fromX, double fromY, double toX, double toY, double radC1, double radC2, double radC3, double radC4, double samples
   ) {
      int color = c.getRGB();
   ...
   private void renderRoundedQuadInternal(
      Matrix4f matrix,
      float cr,
      float cg,
      float cb,
