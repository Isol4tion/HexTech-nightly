...
public class FinalHoleKick extends Module_eSdgMXWuzcxgQVaJFmKZ {
            ...
            if (this.timer.passedMs(this.updateDelay.getValue())) {
               if (!this.selfGround.getValue() || mc.field_1724.method_24828()) {
                  if (this.findBlock(this.getBlockType()) != -1 && this.findClass(PistonBlock.class) != -1) {
                     if (mc.field_1724 == null || !this.noEating.getValue() || !mc.field_1724.method_6115()) {
   ...
   private boolean facingCheck(BlockPos pos) {
      if (!CombatSetting_kxXrLvbWbduSuFoeBUsC.INSTANCE.lowVersion.getValue()) {
         ...
         Direction direction = MathUtil.getDirectionFromEntityLiving(pos, mc.field_1724);
   ...
   private Boolean canPush(PlayerEntity player) {
      if (this.onlyGround.getValue() && !player.method_24828()) {
         return false;
      } else if (!this.allowWeb.getValue() && HoleKickTest.isInWeb(player)) {
         return false;
      } else {
         float[] offset = new float[]{-0.25F, 0.0F, 0.25F};
         int progress = 0;
