...
public class NoSoundLag extends Module_eSdgMXWuzcxgQVaJFmKZ {
   ...
   public void onPlaySound(PlaySoundEvent event) {
      if (this.equip.getValue()) {
         for (SoundEvent se : armor) {
            if (event.sound.method_4775() == se.method_14833()) {
               event.cancel();
               return;
            }
         }
      ...
      if (this.explode.getValue() && event.sound.method_4775() == SoundEvents.field_15152.method_14833()) {
         ...
         if (this.attack.getValue()
            && (event.sound.method_4775() == SoundEvents.field_14625.method_14833() || event.sound.method_4775() == SoundEvents.field_14840.method_14833())) {
            event.cancel();
         }
      }
   }
