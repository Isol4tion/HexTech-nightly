...
extends Module_eSdgMXWuzcxgQVaJFmKZ {
    ...
    public final EnumSetting<BedAura_mDouduXLLBVPsGyiReXU> page = this.add(new EnumSetting<BedAura_mDouduXLLBVPsGyiReXU>("Page", BedAura_mDouduXLLBVPsGyiReXU.General));
    public final EnumSetting<BedAura_uTGCNmVWbsqrhUODnXeN> mode = this.add(new EnumSetting<BedAura_uTGCNmVWbsqrhUODnXeN>("BedMod", BedAura_uTGCNmVWbsqrhUODnXeN.NullPoint));
    ...
    private final BooleanSetting checkMine = this.add(new BooleanSetting("DetectMining", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    ...
    private final EnumSetting<Enum_IKgLeKHCELPvcpdGlLhV> calcMode = this.add(new EnumSetting<Enum_IKgLeKHCELPvcpdGlLhV>("CalcMode", Enum_IKgLeKHCELPvcpdGlLhV.OyVey, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    private final EnumSetting<SwingSide> swingMode = this.add(new EnumSetting<SwingSide>("Swing", SwingSide.Server, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    private final SliderSetting antiSuicide = this.add(new SliderSetting("AntiSuicide", 3.0, 0.0, 10.0, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    private final SliderSetting targetRange = this.add(new SliderSetting("TargetRange", 12.0, 0.0, 20.0, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    private final SliderSetting updateDelay = this.add(new SliderSetting("UpdateDelay", 50, 0, 1000, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    private final SliderSetting calcDelay = this.add(new SliderSetting("CalcDelay", 200, 0, 1000, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    private final BooleanSetting inventorySwap = this.add(new BooleanSetting("InventorySwap", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.General));
    private final BooleanSetting rotate = this.add(new BooleanSetting("Rotation", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Rotate).setParent());
    private final BooleanSetting newRotate = this.add(new BooleanSetting("NewRotate", false, v -> this.rotate.isOpen() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Rotate));
    private final SliderSetting yawStep = this.add(new SliderSetting("YawStep", 0.3f, 0.1f, 1.0, 0.01f, v -> this.rotate.isOpen() && this.newRotate.getValue() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Rotate));
    private final BooleanSetting random = this.add(new BooleanSetting("Random", true, v -> this.rotate.isOpen() && this.newRotate.getValue() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Rotate));
    private final BooleanSetting sync = this.add(new BooleanSetting("Sync", false, v -> this.rotate.isOpen() && this.newRotate.getValue() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Rotate));
    private final BooleanSetting checkLook = this.add(new BooleanSetting("CheckLook", true, v -> this.rotate.isOpen() && this.newRotate.getValue() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Rotate));
    private final SliderSetting fov = this.add(new SliderSetting("Fov", 5.0, 0.0, 30.0, v -> this.rotate.isOpen() && this.newRotate.getValue() && this.checkLook.getValue() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Rotate));
    ...
    private final SliderSetting placeDelay = this.add(new SliderSetting("PlaceDelay", 300, 0, 1000, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Calc && this.place.getValue()));
    ...
    private final SliderSetting breakDelay = this.add(new SliderSetting("BreakDelay", 300, 0, 1000, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Calc && this.Break.getValue()));
    private final BooleanSetting breakOnlyHasCrystal = this.add(new BooleanSetting("OnlyHasBed", false, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Calc && this.Break.getValue()));
    ...
    private final SliderSetting placeMinDamage = this.add(new SliderSetting("MinDamage", 5.0, 0.0, 36.0, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Calc));
    private final SliderSetting placeMaxSelf = this.add(new SliderSetting("MaxSelfDamage", 12.0, 0.0, 36.0, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Calc));
    ...
    private final BooleanSetting shrink = this.add(new BooleanSetting("Shrink", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final BooleanSetting outline = this.add(new BooleanSetting("Outline", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()).setParent());
    private final SliderSetting outlineAlpha = this.add(new SliderSetting("OutlineAlpha", 150, 0, 255, v -> this.outline.isOpen() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final BooleanSetting box = this.add(new BooleanSetting("Box", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()).setParent());
    private final SliderSetting boxAlpha = this.add(new SliderSetting("BoxAlpha", 70, 0, 255, v -> this.box.isOpen() && this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final BooleanSetting reset = this.add(new BooleanSetting("Reset", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final ColorSetting color = this.add(new ColorSetting("Color", new Color(255, 255, 255), v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final SliderSetting animationTime = this.add(new SliderSetting("AnimationTime", 2.0, 0.0, 8.0, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final SliderSetting startFadeTime = this.add(new SliderSetting("StartFadeTime", 0.3, 0.0, 2.0, 0.01, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final SliderSetting fadeTime = this.add(new SliderSetting("FadeTime", 0.3, 0.0, 2.0, 0.01, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Render && this.render.getValue()));
    private final SliderSetting predictTicks = this.add(new SliderSetting("PredictTicks", 4, 0, 10, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Predict));
    private final BooleanSetting terrainIgnore = this.add(new BooleanSetting("TerrainIgnore", true, v -> this.page.getValue() == BedAura_mDouduXLLBVPsGyiReXU.Predict));
    ...
    private void animUpdate() {
        ...
        if (placePos != null) {
            ...
            if (this.renderPos == null || !this.renderPos.equals((Object)placePos)) {
                this.animation.setLength(this.animationTime.getValue() * 1000.0 <= 0.0 ? 0L : (long)(Math.abs(this.nowBB.field_1323 - this.lastBB.field_1323) + Math.abs(this.nowBB.field_1322 - this.lastBB.field_1322) + Math.abs(this.nowBB.field_1321 - this.lastBB.field_1321) <= 5.0 ? (double)((long)((Math.abs(this.nowBB.field_1323 - this.lastBB.field_1323) + Math.abs(this.nowBB.field_1322 - this.lastBB.field_1322) + Math.abs(this.nowBB.field_1321 - this.lastBB.field_1321)) * (this.animationTime.getValue() * 1000.0))) : this.animationTime.getValue() * 5000.0));
        ...
        if (this.nowBB != null && this.lastBB != null) {
            if (Math.abs(this.nowBB.field_1323 - this.lastBB.field_1323) + Math.abs(this.nowBB.field_1322 - this.lastBB.field_1322) + Math.abs(this.nowBB.field_1321 - this.lastBB.field_1321) > 16.0) {
                this.nowBB = this.lastBB;
            }
            if (this.lastSize != size) {
                this.nowBB = new Box(this.nowBB.field_1323 + (this.lastBB.field_1323 - this.nowBB.field_1323) * size, this.nowBB.field_1322 + (this.lastBB.field_1322 - this.nowBB.field_1322) * size, this.nowBB.field_1321 + (this.lastBB.field_1321 - this.nowBB.field_1321) * size, this.nowBB.field_1320 + (this.lastBB.field_1320 - this.nowBB.field_1320) * size, this.nowBB.field_1325 + (this.lastBB.field_1325 - this.nowBB.field_1325) * size, this.nowBB.field_1324 + (this.lastBB.field_1324 - this.nowBB.field_1324) * size);
                this.lastSize = size;
            }
        }
    }
    ...
    private float[] injectStep(float[] angle, float steps) {
        ...
        if (steps < 1.0f && angle != null) {
            ...
            float diff = MathHelper.method_15393((float)(angle[0] - this.lastYaw));
