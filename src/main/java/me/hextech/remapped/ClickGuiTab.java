...
public class ClickGuiTab extends Tab {
   ...
   boolean popped = true;
   private Module_JlagirAibYQgkHtbRnhw category = null;

   public ClickGuiTab(String title, int x, int y) {
      this.title = title;
      this.x = me.hextech.HexTech.CONFIG.getInt(title + "_x", x);
      this.y = me.hextech.HexTech.CONFIG.getInt(title + "_y", y);
      this.width = 100;
   ...
   public void draw(DrawContext drawContext, float partialTicks, Color color) {
      ...
      this.currentHeight = Component.animate(this.currentHeight, (double)this.height);
      if (ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.categoryEnd.booleanValue) {
         Render2DUtil.drawRectVertical(
            matrixStack, (float)this.x, (float)(this.y - 2), (float)this.width, 15.0F, color, ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.categoryEnd.getValue()
         );
      } else {
      ...
      if (this.popped) {
         Render2DUtil.drawRect(
            ...
            (float)(this.y + 14),
            (float)this.width,
            (float)((int)this.currentHeight),
            ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.bgColor.getValue()
         );
      }
      ...
      if (this.popped) {
         ...
         for (Component child : this.children) {
            if (child.isVisible()) {
               ...
               child.currentOffset = (double)(i - 15);
            }
         }
      }
      ...
      TextUtil.drawString(
         drawContext,
         this.title,
         (double)this.x + (double)this.width / 2.0 - (double)(TextUtil.getWidth(this.title) / 2.0F),
         (double)(this.y + 2),
         new Color(255, 255, 255)
      );
      if (this.category != null) {
         String text = "[" + me.hextech.HexTech.MODULE.categoryModules.get(this.category) + "]";
         TextUtil.drawStringWithScale(
            drawContext, text, (float)(this.x + this.width - 4) - TextUtil.getWidth(text) * 0.5F, (float)(this.y + 2), new Color(255, 255, 255), 0.5F
         );
      }
   }
}
