...
public class ModuleComponent extends Component {
   ...
   public void update(int offset, double mouseX, double mouseY, boolean mouseClicked) {
      ...
      this.hovered = mouseX >= (double)parentX
         && mouseX <= (double)(parentX + parentWidth)
         && mouseY >= (double)(parentY + offset)
         && mouseY <= (double)(parentY + offset + this.defaultHeight - 1);
      if (this.hovered && GuiManager.currentGrabbed == null) {
         if (mouseClicked) {
            ClickGuiScreen.clicked = false;
            if (InputUtil.method_15987(mc.method_22683().method_4490(), 340)) {
   ...
   public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
      ...
      if (ClickGui_ABoiivByuLsVqarYqfYv.fade.getQuad(FadeUtils.Out) >= 1.0 && ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.scissor.getValue()) {
         this.setScissorRegion(
            parentX * 2,
            (int)(((double)parentY + this.currentOffset + (double)this.defaultHeight) * 2.0),
            parentWidth * 2,
            mc.method_22683().method_4507() - (int)((double)parentY + this.currentOffset + (double)this.defaultHeight)
         );
      }
      ...
      if (this.popped) {
         ...
         for (Component children : this.settingsList) {
            if (children.isVisible()) {
               ...
               children.currentOffset = (double)(i - this.defaultHeight);
            }
         }
         ...
         for (Component childrenx : this.settingsList) {
            if (childrenx.isVisible()) {
               if (!childrenx.draw((int)this.currentOffset, drawContext, partialTicks, color, true)) {
                  finish = true;
               } else {
                  finish2 = false;
               }
         ...
         for (Component childrenxx : this.settingsList) {
            childrenxx.currentOffset = this.currentOffset;
         }
      }
      ...
      this.currentWidth = animate(
         this.currentWidth, this.module.isOn() ? (double)parentWidth - 2.0 : 0.0, ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.booleanSpeed.getValue()
      );
      if (ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.moduleEnd.booleanValue) {
         Render2DUtil.drawRectHorizontal(
            matrixStack,
            (float)(parentX + 1),
            (float)((int)((double)parentY + this.currentOffset)),
            (float)this.currentWidth,
            (float)(this.defaultHeight - 1),
            ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.moduleEnable.getValue(),
            ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.moduleEnd.getValue()
         );
      } else {
      ...
      Render2DUtil.drawRect(
         ...
         (float)(parentWidth - 2),
         (float)(this.defaultHeight - 1),
         this.hovered ? ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.mhColor.getValue() : ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.mbgColor.getValue()
      );
      if (this.hovered && InputUtil.method_15987(mc.method_22683().method_4490(), 340)) {
         TextUtil.drawString(
            drawContext,
            "Drawn " + (this.module.drawnSetting.getValue() ? "§aOn" : "§cOff"),
            (double)((float)(parentX + 4)),
            (double)((float)((double)parentY + this.getTextOffsetY() + this.currentOffset) - 1.0F),
            -1
         );
      } else {
         TextUtil.drawString(
            ...
            (double)((float)((double)parentY + this.getTextOffsetY() + this.currentOffset) - 1.0F),
            this.module.isOn()
               ? ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.enableText.getValue().getRGB()
               : ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.disableText.getValue().getRGB()
         );
      }
      ...
      if (ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.bindC.booleanValue && this.module.getBind().getKey() != -1) {
         ...
         TextUtil.drawStringWithScale(
            drawContext,
            bindText,
            (float)(parentX + 5) + TextUtil.getWidth(this.text),
            (float)((double)parentY + this.getTextOffsetY() + this.currentOffset - (double)(TextUtil.getHeight() / 4.0F)),
            ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.bindC.getValue(),
            0.5F
         );
      }
      ...
      if (ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.gearColor.booleanValue) {
         if (this.isPopped) {
            TextUtil.drawString(
               drawContext,
               "…",
               (double)(parentX + parentWidth - 12),
               (double)parentY + this.getTextOffsetY() + this.currentOffset - 3.0,
               ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.gearColor.getValue().getRGB()
            );
         } else {
            TextUtil.drawString(
               drawContext,
               "+",
               (double)(parentX + parentWidth - 11),
               (double)parentY + this.getTextOffsetY() + this.currentOffset,
               ClickGui_ABoiivByuLsVqarYqfYv.INSTANCE.gearColor.getValue().getRGB()
            );
         }
      }
