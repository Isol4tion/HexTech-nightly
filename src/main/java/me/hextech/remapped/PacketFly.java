...
extends Module_eSdgMXWuzcxgQVaJFmKZ {
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingEvent event) {
        ...
        double d = PacketFly.mc.field_1724.field_3913.field_3904 && (checkCollisionBoxes || !MovementUtil.isMoving()) ? (this.flight.getValue() && !checkCollisionBoxes ? (this.flightMode.getValue() == 0.0 ? (this.resetCounter(10) ? -0.032 : 0.062) : (this.resetCounter(20) ? -0.032 : 0.062)) : 0.062) : (PacketFly.mc.field_1724.field_3913.field_3903 ? -0.062 : (!checkCollisionBoxes ? (this.resetCounter(4) ? (this.flight.getValue() ? -0.04 : 0.0) : 0.0) : (speed = 0.0)));
    ...
    private boolean checkHitBoxes() {
        return PacketFly.mc.field_1687.method_39454((Entity)PacketFly.mc.field_1724, PacketFly.mc.field_1724.method_5829().method_1009(-0.0625, -0.0625, -0.0625));
    }
    ...
    private double[] getMotion(double speed) {
        ...
        float rotationYaw = PacketFly.mc.field_1724.field_5982 + (PacketFly.mc.field_1724.method_36454() - PacketFly.mc.field_1724.field_5982) * mc.method_1488();
        if (moveForward != 0.0f) {
            if (moveStrafe > 0.0f) {
    ...
    private void sendPackets(double x, double y, double z, boolean teleport) {
        ...
        Vec3d position = PacketFly.mc.field_1724.method_19538().method_1019(vec);
        Vec3d outOfBoundsVec = this.outOfBoundsVec(position);
        this.packetSender((PlayerMoveC2SPacket)new PlayerMoveC2SPacket.PositionAndOnGround(position.field_1352, position.field_1351, position.field_1350, PacketFly.mc.field_1724.method_24828()));
        if (this.invalidPacket.getValue()) {
            this.packetSender((PlayerMoveC2SPacket)new PlayerMoveC2SPacket.PositionAndOnGround(outOfBoundsVec.field_1352, outOfBoundsVec.field_1351, outOfBoundsVec.field_1350, PacketFly.mc.field_1724.method_24828()));
        }
        if (this.setPos.getValue()) {
            PacketFly.mc.field_1724.method_5814(position.field_1352, position.field_1351, position.field_1350);
        }
    ...
    private void teleportPacket(boolean shouldTeleport) {
        if (shouldTeleport) {
            PacketFly.mc.field_1724.field_3944.method_52787((Packet)new TeleportConfirmC2SPacket(++this.teleportID));
        }
    }
    ...
    private void packetSender(PlayerMoveC2SPacket packet) {
        ...
        PacketFly.mc.field_1724.field_3944.method_52787((Packet)packet);
    }
}
